...
public class UserInterface implements ActionListener, Observer
{
    ...
    public void showEnemies(List<Enemy> enemies)
    {
    	for (int i = 0; i < 5; i++)
    	{
    	    ...
    	    {
    	    	ImageIcon icon = new ImageIcon("none"); // dummy value, so no monster graphic
	            monsterSprites.get(i).setIcon(icon);
	            myFrame.pack();
    	    }
    	}
    ...
    /**
     * Just a method to draw the number of shadows that should be present in a room
    ...
    public void showShadows(List<Enemy> enemies)
    {
    	...
    	{
    		ImageIcon icon = new ImageIcon("none"); // dummy value, so no shadow
            shadows.get(i).setIcon(icon);
            myFrame.pack();
    	}
        ...
        if(imageURL == null)
            ...
            ImageIcon icon = new ImageIcon(imageURL);
            shadows.get(5).setIcon(icon); // shadow number 6 (if counting the first as 1) is always the players shadow, the max number of enemies in a room is set to 5
            myFrame.pack();
        }
